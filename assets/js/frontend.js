...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fancyProductDesigner = new FancyProductDesigner(
    $selector.get(0),
    fpd_setup_configs.app_options
  );
  ...
  fancyProductDesigner.addEventListener("ready", () => {
    ...
    if (
      fpd_setup_configs.initial_product &&
      fpd_setup_configs.initial_product.length > 1
    ) {
      var fpdData =
        typeof fpd_setup_configs.initial_product == "string"
          ? JSON.parse(fpd_setup_configs.initial_product)
          : fpd_setup_configs.initial_product;
    ...
    if (fpd_setup_configs.misc.login_required) {
      jQuery("fpd-module-uploads .fpd-upload-image").html(
        '<div class="fpd-login-info">' +
          fpd_setup_configs.labels.login_required +
          "</div>"
      );
    }
    ...
    if (
      fpd_setup_configs.misc.pro_export_enabled &&
      fpd_setup_configs.misc.print_download_designer
    ) {
  ...
  fancyProductDesigner.addEventListener("productCreate", () => {
    ...
    if (
      ["all", "any"].includes(
        fancyProductDesigner.mainOptions.customizationRequiredRule
      )
    ) {
  ...
  fancyProductDesigner.addEventListener("historyAction", () => {
    let customizationChecker = false,
      jsMethod =
        fancyProductDesigner.mainOptions.customizationRequiredRule == "all"
          ? "every"
          : "some";

    customizationChecker = fancyProductDesigner.viewInstances[jsMethod](
      (viewInst) => {
  ...
  if (fpd_setup_configs.misc.store_designs_account) {
    ...
    fancyProductDesigner.addEventListener("ready", () => {
      const gridElem = fancyProductDesigner.container.querySelector(
        "fpd-module-save-load .fpd-grid"
      );
              ...
              Object.keys(response.data).forEach((metaKey) => {
                ...
                const item =
                  fancyProductDesigner[
                    "moduleInstance_save-load"
                  ].addSavedProduct(fpdData);
    ...
    fancyProductDesigner.addEventListener("actionLoad:Remove", (evt) => {
